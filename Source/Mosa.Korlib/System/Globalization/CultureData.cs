...
namespace System.Globalization
{
	...
	internal sealed partial class CultureData
	{
		...
		internal static CultureData? GetCultureDataForRegion(string? cultureName, bool useUserOverride)
		{
			...
			{
				retVal = IcuGetCultureDataFromRegionName(cultureName);
			}
...
#pragma warning restore 618
		...
		internal static CultureData? GetCultureData(string? cultureName, bool useUserOverride)
		{
			...
			{
				if (GlobalizationMode.Invariant || !IcuIsEnsurePredefinedLocaleName(cultureName))
					return null;
			}
		...
		private string GetLanguageDisplayNameCore(string cultureName) => IcuGetLanguageDisplayName(cultureName);
		...
		/// </summary>
		internal string ThreeLetterWindowsLanguageName => _sAbbrevLang ??= IcuGetThreeLetterWindowsLanguageName(_sRealName!);
		...
		/// </summary>
		internal string SCONSOLEFALLBACKNAME => _sConsoleFallbackName ??= IcuGetConsoleFallbackName(_sRealName!);
		...
		internal string TimeSeparator
		{
			get
			{
				if (_sTimeSeparator == null && !GlobalizationMode.Invariant)
				{
					...
					{
						string? longTimeFormat = IcuGetTimeFormatString();
