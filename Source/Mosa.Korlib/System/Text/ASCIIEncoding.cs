...
namespace System.Text
{
	...
	public partial class ASCIIEncoding : Encoding
	{
		...
		internal override sealed void SetDefaultFallbacks()
		{
		...
		protected override sealed unsafe int GetByteCountFast(char* pChars, int charsLength, EncoderFallback? fallback, out int charsConsumed)
		{
		...
		protected override sealed unsafe int GetBytesFast(char* pChars, int charsLength, byte* pBytes, int bytesLength, out int charsConsumed)
		{
		...
		protected override sealed unsafe int GetBytesWithFallback(ReadOnlySpan<char> chars, int originalCharsLength, Span<byte> bytes, int originalBytesLength, EncoderNLS? encoder)
		{
		...
		protected override sealed unsafe int GetCharCountFast(byte* pBytes, int bytesLength, DecoderFallback? fallback, out int bytesConsumed)
		{
			// First: Can we short-circuit the entire calculation?
		...
		protected override sealed unsafe int GetCharsFast(byte* pBytes, int bytesLength, char* pChars, int charsLength, out int bytesConsumed)
		{
		...
		protected override sealed unsafe int GetCharsWithFallback(ReadOnlySpan<byte> bytes, int originalBytesLength, Span<char> chars, int originalCharsLength, DecoderNLS? decoder)
		{
		...
		internal override sealed bool TryGetByteCount(Rune value, out int byteCount)
		{
			if (value.IsAscii)
			{
				byteCount = 1;
		...
		internal override sealed OperationStatus EncodeRune(Rune value, Span<byte> bytes, out int bytesWritten)
		{
			if (value.IsAscii)
			{
				if (!bytes.IsEmpty)
		...
		internal override sealed OperationStatus DecodeFirstRune(ReadOnlySpan<byte> bytes, out Rune value, out int bytesConsumed)
		{
			if (!bytes.IsEmpty)
			{
				byte b = bytes[0];
